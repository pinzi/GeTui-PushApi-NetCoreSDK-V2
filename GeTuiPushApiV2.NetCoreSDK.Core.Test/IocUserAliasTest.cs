...
namespace GeTuiPushApiV2.NetCoreSDK.Core.Test
{
    /// <summary>
    /// IOC方式
    /// </summary>
    public class IocUserAliasTest
    {
        public async Task Run(IServiceCollection services)
        {
            ...
            //1.直接调用API HTTP请求方法，自行处理cid别名的存储
            {
                ...
                var result4 = await GeTuiPushV2Api.HttpDeleteGeTuiApiNoDataAsync<ApiUserAliasUnBoundInDto>($"https://restapi.getui.com/v2/{geTuiPushOptions.AppID}/user/alias/{indto4.alias}", indto4);
            ...
            //3.使用封装好的个推API服务
            {
                ...
                Console.WriteLine(result3.msg);
                //解绑所有
                await service.UserAliasUnBoundAsync(new UserAliasUnBoundInDto()
                {
                    alias = "新用户"
                });
            }
