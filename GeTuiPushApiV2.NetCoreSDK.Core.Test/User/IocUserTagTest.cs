    ...
    public class IocUserTagTest
    {
        public async Task Run(IServiceCollection services)
        {
            ...
            //1.直接调用API HTTP请求方法，自行处理cid别名的存储
            {
                ...
                var result2 = await GeTuiPushV2Api.HttpPutGeTuiApiNoDataAsync<ApiUserTagBatchBindInDto>($"https://restapi.getui.com/v2/{geTuiPushOptions.AppID}/user/custom_tag/batch/{indto2.custom_tag}", indto2);
            ...
            //3.使用封装好的个推API服务
            {
                ...
                await service.UserTagBatchBindAsync(indto2);
                Thread.Sleep(1000);
                //查询用户标签
