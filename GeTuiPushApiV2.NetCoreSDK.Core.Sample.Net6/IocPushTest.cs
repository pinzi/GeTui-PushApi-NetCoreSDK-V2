    ...
    public class IocPushTest
    {
        public async Task Run(IServiceCollection services)
        {
            ...
            //1.直接调用API HTTP请求方法，需要自己实现Token的缓存，CID的保存及查询逻辑
            {
                ...
                var result2 = await GeTuiPushV2Api.HttpPostGeTuiApiAsync<ApiPushToSingleCIDInDto, Dictionary<string, Dictionary<string, string>>>($"https://restapi.getui.com/v2/{geTuiPushOptions.AppID}/push/single/cid", apiInDto);
