...
**生产环境强烈建议使用环境变量存储敏感配置**，避免将密钥硬编码到代码或配置文件中。

**设置环境变量：**

- **Windows (PowerShell)**:
  ```powershell
  $env:GETUI_APP_ID="your_app_id"
  $env:GETUI_APP_KEY="your_app_key"
  $env:GETUI_MASTER_SECRET="your_master_secret"
  ```

- **Windows (CMD - 永久生效)**:
  ```cmd
  setx GETUI_APP_ID "your_app_id"
  setx GETUI_APP_KEY "your_app_key"
  setx GETUI_MASTER_SECRET "your_master_secret"
  ```
...
- **Azure App Service / AWS Elastic Beanstalk 等云平台**:
  在控制台的"应用设置"或"环境变量"中添加上述变量。

#### 方式二：使用配置文件（仅限开发环境）

⚠️ **警告**: 不要将包含真实密钥的配置文件提交到版本控制系统！
...
对于生产环境，请使用 `appsettings.Production.json` 并确保该文件被 `.gitignore` 忽略。

### 3. 注册服务（IOC 方式）

在 `Program.cs` 或 `Startup.cs` 中注册服务，SDK 会自动从环境变量读取配置：
...
// 从环境变量读取配置（推荐）
var appId = Environment.GetEnvironmentVariable("GETUI_APP_ID");
var appKey = Environment.GetEnvironmentVariable("GETUI_APP_KEY");
var masterSecret = Environment.GetEnvironmentVariable("GETUI_MASTER_SECRET");
