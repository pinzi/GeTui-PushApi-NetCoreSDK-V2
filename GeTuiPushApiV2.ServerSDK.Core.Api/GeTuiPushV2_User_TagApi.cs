namespace GeTuiPushApiV2.ServerSDK.Core
{
    ...
    public partial class GeTuiPushV2Api
    {
        ...
        /// <param name="inDto"></param>
        /// <returns></returns>
        public async Task<ApiResultOutDto<ApiUserTagBatchBindOutDto>> UserTagBatchBindAsync(ApiUserTagBatchBindInDto inDto)
        {
            var result = await HttpPutGeTuiApiAsync<ApiUserTagBatchBindInDto, ApiUserTagBatchBindOutDto>($"{ApiBaseUrl}{inDto.appId}/user/custom_tag/batch/{inDto.custom_tag}", inDto);
            return result;
        }
        #endregion

        #region 【标签】一批用户解绑一个标签
        /// <summary>
        /// 用户-【标签】一批用户解绑一个标签
        /// 解绑用户的某个标签属性，不影响其它标签
        /// 此接口有频次控制(每分钟最多100次，每天最多10000次)，申请修改请点击右侧“技术咨询”了解详情
